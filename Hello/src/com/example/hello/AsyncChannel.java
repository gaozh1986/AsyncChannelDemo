...
public class AsyncChannel {
	...
	/**
	 * Command sent when the channel is half connected. Half connected means
	 * that the channel can be used to send commends to the destination but the
	 * destination is unaware that the channel exists. The first command sent to
	 * the destination is typically CMD_CHANNEL_FULL_CONNECTION if it is desired
	 * to establish a long term connection, but any command maybe sent.
	 * 
	 * msg.arg1 == 0 : STATUS_SUCCESSFUL 1 : STATUS_BINDING_UNSUCCESSFUL msg.obj
	 * == the AsyncChannel msg.replyTo == dstMessenger if successful
	 */
	public static final int CMD_CHANNEL_HALF_CONNECTED = BASE + 0;

	/**
	 * Command typically sent when after receiving the
	 * CMD_CHANNEL_HALF_CONNECTED. This is used to initiate a long term
	 * connection with the destination and typically the destination will reply
	 * with CMD_CHANNEL_FULLY_CONNECTED.
	 * 
	 ...
	 * CMD_CHANNEL_FULL_CONNECTION. This signifies the acceptance or rejection
	 * of the channel by the sender.
	 * 
	 * msg.arg1 == 0 : Accept connection : All other values signify the
	 * destination rejected the connection and {@link AsyncChannel#disconnect}
	 ...
	 * {@link AsyncChannel#disconnect} to close its side of the channel and it
	 * will receive a CMD_CHANNEL_DISCONNECTED when the channel is closed.
	 * 
	 ...
	 * 
	 * msg.arg1 == 0 : STATUS_SUCCESSFUL 1 : STATUS_BINDING_UNSUCCESSFUL 2 :
	 * STATUS_SEND_UNSUCCESSFUL : All other values signify failure and the
	 * channel state is indeterminate msg.obj == the AsyncChannel msg.replyTo =
	 * messenger disconnecting or null if it was never connected.
	 */
	public static final int CMD_CHANNEL_DISCONNECTED = BASE + 4;

	private static final int CMD_TO_STRING_COUNT = CMD_CHANNEL_DISCONNECTED
			- BASE + 1;
	 ...
	 * @param srcHandler
	 *            is the hander to receive CONNECTED & DISCONNECTED messages
	 * @param dstPackageName
	 *            is the destination package name
	 * @param dstClassName
	 *            is the fully qualified class name (i.e. contains package name)
	 * 
	 * @return STATUS_SUCCESSFUL on success any other value is an error.
	 */
	public int connectSrcHandlerToPackageSync(Context srcContext,
			Handler srcHandler, String dstPackageName, String dstClassName) {
		if (DBG)
		...
		/*
		 * Initialize destination information to null they will be initialized
		 * when the AsyncChannelConnection#onServiceConnected is called
		 */
		mDstMessenger = null;
	 ...
	 * @return STATUS_SUCCESSFUL on success any other value is an error.
	 */
	public int connectSync(Context srcContext, Handler srcHandler,
	 ...
	 * @return STATUS_SUCCESSFUL on success any other value is an error.
	 */
	public int fullyConnectSync(Context srcContext, Handler srcHandler,
			Handler dstHandler) {
	 ...
	 * msg.arg1 = status msg.obj = the AsyncChannel
	 * 
	 * @param srcContext
	 *            is the context of the source
	 * @param srcHandler
	 ...
	 *            is the fully qualified class name (i.e. contains package name)
	 */
	public void connect(Context srcContext, Handler srcHandler,
		...
		final class ConnectAsync implements Runnable {
			...
			ConnectAsync(Context srcContext, Handler srcHandler,
					String dstPackageName, String dstClassName) {
				mSrcCtx = srcContext;
				mSrcHdlr = srcHandler;
			...
			@Override
			public void run() {
				int result = connectSrcHandlerToPackageSync(mSrcCtx, mSrcHdlr,
						mDstPackageName, mDstClassName);
				replyHalfConnected(result);
			}
		}

		ConnectAsync ca = new ConnectAsync(srcContext, srcHandler,
				dstPackageName, dstClassName);
		new Thread(ca).start();
	 ...
	 * @param srcHandler
	 * @param klass
	 *            is the class to send messages to.
	 */
	public void connect(Context srcContext, Handler srcHandler, Class<?> klass) {
		connect(srcContext, srcHandler, klass.getPackage().getName(),
				klass.getName());
	}
	...
	/**
	 * Connect handler to messenger. This method is typically called when a
	 * server receives a CMD_CHANNEL_FULL_CONNECTION request and initializes the
	 * internal instance variables to allow communication with the dstMessenger.
	 * 
	 * @param srcContext
	 * @param srcHandler
	 * @param dstMessenger
	 */
	public void connected(Context srcContext, Handler srcHandler,
			Messenger dstMessenger) {
		if (DBG)
	...
	/**
	 * To close the connection call when handler receives
	...
	public void disconnect() {
		...
		if (mConnection == null && mDstMessenger != null
				&& mDeathMonitor != null) {
	...
	public Message sendMessageSynchronously(Message msg) {
		Message resultMsg = SyncMessenger.sendMessageSynchronously(
				mDstMessenger, msg);
		return resultMsg;
	}
	...
	private static class SyncMessenger {
		...
		private static SyncMessenger obtain() {
			...
			synchronized (sStack) {
				if (sStack.isEmpty()) {
					...
					sm.mHandlerThread = new HandlerThread("SyncHandler-"
							+ sCount++);
					sm.mHandlerThread.start();
					sm.mHandler = sm.new SyncHandler(
							sm.mHandlerThread.getLooper());
		 ...
		 */
		private static Message sendMessageSynchronously(Messenger dstMessenger,
				Message msg) {
	...
	/**
	 * Reply to the src handler that we're half connected. see:
	 * CMD_CHANNEL_HALF_CONNECTED for message contents
	 * 
	 * @param status
	...
	/**
	 * Reply to the src handler that we are disconnected see:
	 * CMD_CHANNEL_DISCONNECTED for message contents
	 * 
	 * @param status
	 *            to be stored in msg.arg1
	 */
